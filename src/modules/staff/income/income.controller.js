...
import { getIncomeHistory, getIncomeStats } from "./income.service.js";
...
export const getIncomeHistoryHandler = asyncHandler(async (req, res) => {
  const lang = getLanguage(req);
  const history = await getIncomeHistory(req.user.sub);
  successResponse(res, 200, t(tr.INCOME_RETRIEVED_SUCCESSFULLY, lang), history);
});
